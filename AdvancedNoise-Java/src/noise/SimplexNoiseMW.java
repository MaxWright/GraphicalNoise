...
public class SimplexNoiseMW {
	...
	/**
	 * A multiplier to make the values closer to [-1, 1]
	 */
	private final int CONSTANT = 34;
	 ...
	 */
	public SimplexNoiseMW(int y, int height) {
		rows = y;
		columns = y;
		rowHeight = height;
		columnWidth = height;
	...
	void calcCoordinateUp(int row, int column, int y, int x) {
		...
		result[row * rowHeight + y][column * columnWidth + x] = (tlCorner + trCorner + blCorner) * CONSTANT;
	}
	...
	void calcCoordinateDown(int row, int column, int y, int x) {
		...
		result[row * rowHeight + y][column * columnWidth + x] = (trCorner + brCorner + blCorner) * CONSTANT;
	}
	...
	void calcCoordinateDivide(int row, int column, int y, int x) {
		...
		double trCorner = dotProductAndWeight(gradients[row][column + 1], tl2PosY, -(1 - tl2PosX));
		double blCorner = dotProductAndWeight(gradients[row + 1][column], -(1 - tl2PosY), tl2PosX);
		result[row * rowHeight + y][column * columnWidth + x] = (blCorner + trCorner) * CONSTANT;
	}
	...
	double dotProductAndWeight(int gradientVectorType, double y, double x) {
		...
		/*
		 * Same as: toReturn^4 * (dot product of gradient and distance vector).
		 */
		toReturn *= toReturn;
		toReturn *= toReturn * (gradientVals[gradientVectorType][0] * y + gradientVals[gradientVectorType][1] * x);
		return toReturn;
	}
