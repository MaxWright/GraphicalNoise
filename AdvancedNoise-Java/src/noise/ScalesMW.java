...
public class ScalesMW {
	 ...
	 */
	public ScalesMW(int y, int height) {
		rows = y;
		columns = y;
		rowHeight = height;
		columnWidth = height;
	...
	void calcCoordinateUp(int row, int column, int y, int x) {
		 ...
		 * calculated.
		 */
		double tlCorner = dotProductAndWeight(gradients[row][column], tl2PosY,
				tl2PosX);
		double trCorner = dotProductAndWeight(gradients[row][column + 1],
				tl2PosY, -(1 - tl2PosX));
		double blCorner = dotProductAndWeight(gradients[row + 1][column],
				-(1 - tl2PosY), tl2PosX);
		result[row * rowHeight + y][column * columnWidth + x] = tlCorner
				+ trCorner + blCorner;
	}
	...
	void calcCoordinateDown(int row, int column, int y, int x) {
		...
		double blCorner = dotProductAndWeight(gradients[row + 1][column],
				-(1 - tl2PosY), tl2PosX);
		double brCorner = dotProductAndWeight(gradients[row + 1][column + 1],
				-(1 - tl2PosY), -(1 - tl2PosX));

		result[row * rowHeight + y][column * columnWidth + x] = trCorner
				+ brCorner + blCorner;
	}
	...
	void calcCoordinateDivide(int row, int column, int y, int x) {
		...
		double blCorner = dotProductAndWeight(gradients[row + 1][column],
				-(1 - tl2PosY), tl2PosX);
		result[row * rowHeight + y][column * columnWidth + x] = blCorner
				+ trCorner;
	}
	...
	double dotProductAndWeight(int gradientVectorType, double y, double x) {
		/*
		 *  Apply some weight to the length of the distance vector.
		 */
		double toReturn = (0.6 - y * y - x * x);
		/*
		 * If the weight is less than zero after applying, don't bother.
		 */
		if(toReturn < 0) {
			toReturn = 0;
		}
		...
		y = fade(y);
		x = fade(x);
		toReturn *= (gradientVals[gradientVectorType][0] * y + gradientVals[gradientVectorType][1]
				* x);
		return toReturn;
	}
